...
def model_maker(model_name, num_classes):
...
def train(model_name, num_classes, trainloader, testloader, epochs):
    ...
    for epoch in range(epochs):  
        ...
        if test_accuracy > best_metric:
            ...
            torch.save(model, "models/train_{}.pth".format(model_name))
...
def watermark(combined_loader, wm_loader, epochs, model_name=None, model_path = None):   
    ...
    if model_path is not None:
        ...
        #model = model_maker(model_name, num_classes)
        model = torch.load("models/train_{}.pth".format(model_name))
    else:
    ...
    for epoch in range(epochs):  
        ...
        if test_accuracy > wm_metric:
            ...
            #best_metric_epoch = epoch + 1
            torch.save(model, "models/wm_{}.pth".format(model_name))
...
#     target_label = 4
    ...
    if model_name == None:
        ...
        model = train(model_name=model_name, num_classes=num_classes, trainloader = trainloader, testloader = testloader, epochs=int(train_epochs))
...
#     target_label = 4
    ...
    combined_set = ConcatDataset([trainset, trig_set])
    combined_loader = DataLoader(combined_set, batch_size=4, shuffle=True)
    wm_model = watermark(model_name=model_name, model_path = model_path, combined_loader=combined_loader, wm_loader = wm_loader, epochs=wm_epochs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', help="Choose one among:['train_and_embed', 'embed','verify']", default="train_and_embed")
    ...
    if task == 'train_and_embed':
        ...
        print("Please choose a task among: ['train_and_embed', 'embed', 'verify']")
